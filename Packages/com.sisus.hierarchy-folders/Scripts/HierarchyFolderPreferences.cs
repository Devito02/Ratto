...
namespace Sisus.HierarchyFolders
{
...
#endif
	{
		...
		public Icon GetDefaultIcon(int size = 16)
		{
			...
			EditorGUIUtility.SetIconSize(new(size, size));
			var icon = new Icon();
		...
		{
			allowComponentsByNameOnHierarchyFolders = new(whitelistedComponents);
			allowComponentsByTypeOnHierarchyFolders = new();
		...
		{
			if(!this || defaultIconsSuccessfullyLoaded)
			{
				return;
			}

			if(prefabModernDark.closed)
			{
