...
namespace Sisus.HierarchyFolders
{
	...
	public static class HierarchyFolderIconDrawer
	{
		...
		private static void DrawIcon(int instanceId, Rect itemRect)
		{
			...
			if(!gameObject || !HierarchyFolderUtility.IsRegisteredHierarchyFolder(gameObject))
			{
				return;
			}
			...
			var isFocused = HierarchyWindowUtility.IsFocused(instanceId);
			...
			var isPrefabInstanceRoot = gameObject.IsPrefabInstanceRoot();
			var parentIsPartOfPrefabInstance = transform.parent && transform.parent.gameObject.IsConnectedPrefabInstance();
			var drawAddedOverlay = false;
			...
			if(isPrefabInstanceRoot)
			{
				...
				if(parentIsPartOfPrefabInstance)
				{
					if(isPrefabVariant)
					{
						icon = expanded ? prefabVariantAdditionFolderIconOpen : prefabVariantAdditionFolderIconClosed;
					}
					else
					{
						icon = expanded ? prefabAdditionFolderIconOpen: prefabAdditionFolderIconClosed;
					}
				}
				else if(isPrefabVariant)
				{
					icon = expanded ? prefabVariantFolderIconOpen : prefabVariantFolderIconClosed;
				}
				else
				{
					icon = expanded ? prefabFolderIconOpen : prefabFolderIconClosed;
				}
			}
			else if(gameObject.TryGetComponent(out HierarchyFolder hierarchyFolder) && hierarchyFolder.Color != Color.white)
			{
				color = hierarchyFolder.Color;
				color.a = 1f;
				icon = expanded ? defaultIconOpen : defaultIconClosed;
				drawAddedOverlay = parentIsPartOfPrefabInstance && !gameObject.IsConnectedPrefabInstance();
			}
